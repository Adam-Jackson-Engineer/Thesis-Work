"""
pid_path_euler_torque_leader_pos.py

A 6 DoF PID controllers for an agent that knows the reference position, it finds desired torques
in the euler frames, then converts them to the inertial frame. This controller controlls the leader's
position rather than the center of the table.
...
class PIDPathEulerTorqueLeaderPos:
    """
    ...
    def __init__(self):
        ...
        initial_rotation = tp.rot_zyx(tp.PHI_0, tp.THETA_0, tp.PSI_0)
        initial_leader_offset = initial_rotation.T @ tp.LEADER_FORCE_LOCATION

        self.x_d1 = tp.X_0 + initial_leader_offset[0][0]
        self.y_d1 = tp.Y_0 + initial_leader_offset[1][0]
        self.z_d1 = tp.Z_0 + initial_leader_offset[2][0]
    ...
    def update(self, reference_states, states):
        ...
        reference_rotation = tp.rot_zyx(reference_phi, reference_theta, reference_psi)
        reference_leader_offset = reference_rotation.T @ tp.LEADER_FORCE_LOCATION

        # Move the reference x, y, z to the leaders position
        reference_x += reference_leader_offset[0][0]
        reference_y += reference_leader_offset[1][0]
        reference_z += reference_leader_offset[2][0]
        ...
        state_leader_offset = state_rotation.T @ tp.LEADER_FORCE_LOCATION

        # Move the x, y, z to the leaders position
        x += state_leader_offset[0][0]
        y += state_leader_offset[1][0]
        z += state_leader_offset[2][0]
