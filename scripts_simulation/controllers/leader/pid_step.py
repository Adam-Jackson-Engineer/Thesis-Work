...
A 6 DoF PID controllers for an agent that knows the reference position
"""
...
class PIDStep:
    ...
    def update(self, reference_states, states):
        ...
        force_x_body_frame = saturate(force_body_frame[0][0],tp.FORCE_X_MAX_LEADER)
        force_y_body_frame = saturate(force_body_frame[1][0],tp.FORCE_Y_MAX_LEADER)
        force_Z_body_frame = saturate(force_body_frame[2][0],tp.FORCE_Z_MAX_LEADER)
        torque_phi_body_frame = saturate(torque_body_frame[0][0],tp.TORQUE_PHI_MAX_LEADER)
        torque_theta_body_frame = saturate(torque_body_frame[1][0],tp.TORQUE_THETA_MAX_LEADER)
        torque_psi_body_frame = saturate(torque_body_frame[2][0],tp.TORQUE_PSI_MAX_LEADER)
